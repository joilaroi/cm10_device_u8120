...
int AudioHardware::get_sound_endpoints(void)
{
    ...
    if (rc >= 0) {
        ...
        for (int cnt = 0; cnt < mNumSndEndpoints; cnt++, ept++) {
            ...
            ALOGV("cnt = %d ept->name = %s ept->id = %d", cnt, ept->name, ept->id);
...
status_t AudioHardware::setVoiceVolume(float v)
{
    ...
    ALOGI("Setting in-call volume to %d (available range is 0 to 8)", vol);
...
{
    unsigned int VolValue = (unsigned int)(AudioSystem::ALOGToLinear(v));
